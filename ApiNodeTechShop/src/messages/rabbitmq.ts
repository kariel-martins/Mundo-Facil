// messages/rabbitmq.ts
import amqp, { Channel, ConfirmChannel, Connection, Options } from "amqplib";
...
export async function assertQueueWithDLQ(queue: string, dlx: string, deadLetterRoutingKey = `${queue}.dead`) {
...
export async function publish(
  ...
  return new Promise<void>((resolve, reject) => {
    const ok = ch.publish(exchange, routingKey, payload, { persistent: true, contentType: "application/json", ...options });
    if (!ok) {

...
import { Link } from "react-router-dom"; // Importado Link para o botão de carrinho vazio
...
export function Cart() {
  ...
  async function checkoutCart() {
    cartItems?.map((item)=> {
      createdOrder({product_id: item.products.id, store_id: item.products.store_id, quantity: item.carts.quantity, user_id: item.carts.user_id, status: "Processando"})
    })
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6 pt-28 max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          // Estado de carrinho vazio (Clean/Azul Claro)
          <div className="bg-white p-16 rounded-xl shadow-lg text-center border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">Explore o nosso catálogo para encontrar ofertas incríveis!</p>
            <Link to="/catalogo">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cartItems?.map((item) => (
                <Card 
                  key={item.carts.id}
                  // Card clean: branco, sombra suave, azul claro no hover
                ...
                >
                  <CardContent className="flex justify-between items-center py-5 px-6">
                    ...
                    {/* Controle de Quantidade (Clean Buttons) */}
                    ...
                    {/* Botão de Remover (Vermelho de Alerta) */}
                    <Button
                      onClick={() => removeItenCart(item.carts.id)}
                      variant="ghost"
                      size="icon"
                      // Estilo de remoção limpo, mas com cor de alerta
            ...
            {/* Coluna de Resumo da Compra */}
            <Card className="p-6 space-y-5 h-fit lg:col-span-1 shadow-xl border border-gray-200 sticky top-28">
              <h2 className="text-2xl font-bold text-gray-800 border-b pb-3">Resumo da Compra</h2>
              ...
              <div className="border-t border-gray-200 pt-4">
                 <p className="flex justify-between text-2xl font-extrabold text-gray-800">
                    <span>Total</span>
                    <span className="text-blue-500">R$ {subtotal.toFixed(2)}</span>
                 </p>
              </div>

              {/* Botão de Finalizar Compra (Azul Claro Principal) */}
              <Button 

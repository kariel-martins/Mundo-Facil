...
import { type SignInFormData, validateSignIn } from "../hooks/auth/dtos/schemas";
...
export function SignIn() {
  ...
  async function handleSignIn(data: SignInFormData) {
    try {
      ...
      localStorage.setItem("user", JSON.stringify({user_id: result.user_id, email: result.email, name: result.name}))
      navigate(`/main`);
      signIn.reset();
    } catch (error: any) {
      if (error.response) {
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gray-50">
      ...
      <Card className="w-full max-w-md shadow-xl border border-gray-200">
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-2xl text-gray-800">Acessar Sua Conta</CardTitle>
